...
class MeshOptimizer:
    ...
    def step(self, x):
        ...
        for factor in [1.0, 0.5, 0.25, 0.1, 0.01]:
            x_new = x - self.alpha * factor * dx
            E_new = self.fem.energy.energy(x_new)
            if E_new < E0:
                return x_new
        # if no successful step, fallback to gradient
        return x - self.alpha * g
...
def run_optimization(mesh_data, energy_cls, method='gradient', iters=5, perturb=False):
    V_orig, F = mesh_data
    # perturb initial positions only for display and optimization start
    if perturb and energy_cls is SpringEnergy:
        np.random.seed(42)
        V_init = V_orig + 0.1 * (np.random.rand(*V_orig.shape) - 0.5)
    else:
        V_init = V_orig.copy()
    # build energy from original unperturbed mesh so rest lengths != perturbed lengths
    fem = FEMMesh((V_orig, F), energy_cls)
